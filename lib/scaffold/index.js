...
function run(){
    ...
    start(function(answer){
        ...
        var child = exec(["git clone --depth 1 git@github.com:mjbp/storm-scaffold.git ",
                    santise(projectName),
                    " && cd ./",
                    santise(projectName),
                    " && rm -rf .git ",
        ...
        child.on('close', function(code) {
            ...
            console.log(chalk.yellow(['"cd ', santise(projectName), '" into the new project, then "gulp start" to run it'].join('')));
        });
